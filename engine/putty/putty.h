...
/* Static variables used in all environments. Symbian OS does not allow any
 * static writable data in DLLs, so we can't just use normal static variables.
 * The ideal solution would be just to eliminate static data altogethers, since
 * this would improve re-entrancy, but currently some of the interfaces in
 * PuTTY (random number generator, fatalbox(), old_keyfile_warning(), ...) just
 * don't support this, and I want to avoid changing the interfaces as much as
 * possible. To access a static variable, use
 *      statics()->variable
 * instead of
 *      variable
 * The statics() function/macro will be defined in the platform-specific
 * headers.
 * -- Petteri
*/
#include "tree234.h"
...
extern const struct backend_list {
    int protocol;
    const char *name;
 ...
 */
void *ldisc_create(Config *, Terminal *, const Backend *, void *, void *);
