...
static int compare_timers(void *av, void *bv)
{
    ...
    long at = a->now - statics()->now;
    long bt = b->now - statics()->now;
...
/*
 * [Petteri] Free memory allocated to timers
 */
void free_timers(void)
{
    if ( statics()->timer_contexts ) {
        freetree234(statics()->timer_contexts);
        statics()->timer_contexts = NULL;
    }

    if ( statics()->timers ) {
        struct timer *t;
        while ( (t = index234(statics()->timers, 0)) != NULL ) {
            del234(statics()->timers, t);
            sfree(t);
        }
